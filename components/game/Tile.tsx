...
const isCornerTile = (id: number) =>
  id === 0 || id === 9 || id === 18 || id === 27;
...
export default function TileComponent({
  ...
  const ownerPlayer = allPlayers.find((p) =>
    p.properties.some((pr) => pr.tileId === tile.id),
  );
  ...
  return (
    <div
      className={cn(
        ...
        isActive &&
          "ring-2 ring-yellow-400 ring-offset-1 shadow-lg shadow-yellow-400/40 scale-105 z-10",
      )}
    >
      ...
      {tile.type === "CITY" && tile.color && (
        <div
          className={cn(
            "shrink-0",
            orientation === "top"
              ? "order-last w-full h-2"
              : orientation === "left"
                ? "absolute left-0 top-0 w-2 h-full"
                : orientation === "right"
                  ? "absolute right-0 top-0 w-2 h-full"
      ...
      <div className="flex flex-col items-center justify-center flex-1 px-0.5 py-0.5 gap-0.5">
        {tile.type === "CITY" ? (
          <>
            {tile.flag && (
              <span className="text-lg leading-none">{tile.flag}</span>
            )}
            <span
              className={cn(
                "text-[7px] font-bold text-center leading-tight break-all text-card-foreground",
                corner && "text-[8.5px]",
              )}
            >
              {tile.name}
      ...
      {playersOnTile.length > 0 && (
        <div className="absolute inset-0 flex flex-wrap items-center justify-center gap-0.5 pointer-events-none z-20">
          {playersOnTile.map((p) => (
            <PlayerToken key={p.id} playerId={p.id} size="sm" />
          ))}
        </div>
      )}
    </div>
