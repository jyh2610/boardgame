...
export async function POST(
  ...
  try {
    ...
    switch (action) {
      case 'roll':
        if (state.phase !== 'ROLL') {
          return NextResponse.json({ error: '주사위를 굴릴 수 없습니다.' }, { status: 400 });
        }
      ...
      case 'move':
        ...
        nextState = movePlayer(state, playerId, payload.steps);
        break;

      case 'finishMoving':
        if (state.phase !== 'MOVING') {
          return NextResponse.json({ error: '이동 완료할 수 없습니다.' }, { status: 400 });
        }
      ...
      case 'buy':
        if (state.phase !== 'BUY') {
          return NextResponse.json({ error: '매수할 수 없습니다.' }, { status: 400 });
        }
      ...
      case 'decline':
        if (state.phase !== 'BUY') {
          return NextResponse.json({ error: '거절할 수 없습니다.' }, { status: 400 });
        }
      ...
      case 'payRent':
        if (state.phase !== 'PAY_RENT') {
          return NextResponse.json({ error: '통행료를 낼 수 없습니다.' }, { status: 400 });
        }
      ...
      case 'confirmGoldenKey':
        if (state.phase !== 'GOLDEN_KEY') {
          return NextResponse.json({ error: '황금열쇠를 확인할 수 없습니다.' }, { status: 400 });
        }
      ...
      case 'jailEscape':
        if (state.phase !== 'JAIL') {
          return NextResponse.json({ error: '보석금을 낼 수 없습니다.' }, { status: 400 });
        }
      ...
      case 'confirmModal':
        if (!['TAX', 'WELFARE', 'TRAVEL', 'JAIL'].includes(state.phase)) {
          return NextResponse.json({ error: '모달을 확인할 수 없습니다.' }, { status: 400 });
        }
